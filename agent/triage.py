...
from agent.tools import lookup_rubric, set_retriever
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--retrieval", choices=["keyword", "vector"], default="keyword", help="Retrieval backend for search_reference. Default: keyword.")
    ...
    # Install the retrieval backend. Vector uses Vertex embeddings live, or a
    # MockEmbedder under --mock so the wiring works without credentials.
    if args.retrieval == "vector":
        try:
            embedder = MockEmbedder() if args.mock else None
            set_retriever(build_retriever("vector", config, _tools._REFERENCE_CORPUS["docs"], embedder=embedder))
            print("Retrieval backend: vector", file=sys.stderr)
        except Exception as e:
            print(f"Could not initialize the vector retriever: {e}", file=sys.stderr)
